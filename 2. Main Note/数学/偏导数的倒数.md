...
$$
当然，也可以将等式化为完全显式 $r = \sqrt{x^2 + y^2 + z^2}$ 然后再求导，但是会稍微麻烦一些。由 $(1)$ 式可知 $x=r\sin\theta \cos\phi$ ，代入 $(3)$ 式后得到 $\frac{\partial r}{\partial x}=\sin\theta \cos\phi$，由于 $xyz$ 三者独立且满足加法交换律，所以可以将 $x$ 自由轮换为 $y 、 z$ 。接着对 $(2)$ 式中第二项左右同时对 $x$ 求偏导，得到：
$$
